...
export class InfEstadoLiquidacionesComponent implements OnInit {
  ...
  onGenerar(){
    ...
    const headers = [{periodo: 'PERIODO', concepto: 'CONCEPTO', abonos: 'ABONOS', cargos: 'CARGOS.', saldoliq: 'SALDO LIQ.', fechapago: 'FECHA PAGO'}];
        ...
        .subscribe(data => {
          const data_inf = data[0]?.liquidaciones?.length > 0 ?  data[0]?.liquidaciones.map(liq => {
            return liq.abonos.map(abono => { abono.abono = true; return abono}).concat(liq.cargos)
                      .map((con, idx) => ({periodo: idx == 0 ? this.formatDate(liq.fecha) : '', concepto: con.concepto, abonos: con.abono ? con.valor : '',
                      cargos: con.abono ? '' : con.valor, saldoliq:'', fechapago:''}))
                    .concat({periodo: '', concepto: 'Totales', abonos: liq.totalAbonos, cargos: liq.totalCargos,
                                saldoliq: liq.subtotal, fechapago: this.formatDate(liq.fechapago)})
                    .concat({periodo: '', concepto: '', abonos:'', cargos:'', saldoliq: '', fechapago: ''})
          }).flat() : [];

          this.outputFileName = `ResumenLiquidaciones_${this.formatDate(this.dateEnd, '')}.pdf`;
