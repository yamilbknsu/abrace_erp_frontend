...
export class numberToWordsService {
    ...
    Millones(num) {
        ...
        var strMillones = this.Seccion(num, divisor, "UN MILLON" + (resto == 0 ? ' DE':''), "MILLONES"+ (resto == 0 ? ' DE':''));
    ...
    NumeroALetras(num) {
        var data = {numero: num, enteros: Math.floor(num), centavos: (((Math.round(num * 100)) - (Math.floor(num) * 100))),
            letrasCentavos: "",
